...
def verificar_ejemplos_texto_consolidado(df):
    """
    Verifica ejemplos específicos del texto consolidado por tipo de viaje.
    """
    ...
    for origen in origenes_especificos:
        ...
        registros_origen = df[df['OrigenAutor'] == origen].head(1)
    ...
    if 'texto_consolidado' in df.columns:
        ...
        print(f"📊 Total de registros con texto consolidado: {len(df)}")
        print(f"📏 Longitud promedio: {df['texto_consolidado'].str.len().mean():.1f} caracteres")
