...
def main():
    ...
    try:
        ...
        if test_passed:
            tests_passed += 1
        print_result(
            'Bridge initialization (ready signal)', test_passed, f'Got: {ready_response}' if not test_passed else ''
        )

        # Test 2: Ping command
        tests_total += 1
        response = send_command(proc, {'action': 'ping'})
        test_passed = response.get('success') is True and response.get('message') == 'pong'
        if test_passed:
            tests_passed += 1
        print_result('Ping command', test_passed, f'Got: {response}' if not test_passed else '')

        # Test 3: Get status
        tests_total += 1
        response = send_command(proc, {'action': 'get_status'})
        test_passed = response.get('success') is True and response.get('isRunning') is False
        if test_passed:
            tests_passed += 1
        print_result('Get status command', test_passed, f'Got: {response}' if not test_passed else '')

        # Test 4: Check Ollama
        tests_total += 1
        response = send_command(proc, {'action': 'check_ollama'})
        test_passed = response.get('success') is True
        if test_passed:
            tests_passed += 1
        ollama_status = 'running' if response.get('running') else 'not running'
        print_result(
            f'Check Ollama (Ollama is {ollama_status})', test_passed, f'Got: {response}' if not test_passed else ''
        )
        ...
        if test_passed:
            tests_passed += 1
        print_result('Get LLM info command', test_passed, f'Got: {response}' if not test_passed else '')

        # Test 6: Unknown action
        tests_total += 1
        response = send_command(proc, {'action': 'unknown_action'})
        test_passed = response.get('success') is False and 'Unknown action' in response.get('error', '')
        if test_passed:
            tests_passed += 1
        print_result('Unknown action error handling', test_passed, f'Got: {response}' if not test_passed else '')
        ...
        test_passed = response.get('success') is False
        if test_passed:
            tests_passed += 1
        print_result('Validate dataset (non-existent file)', test_passed, f'Got: {response}' if not test_passed else '')
        ...
        if test_passed:
            tests_passed += 1
        print_result('Stop command', test_passed, f'Got: {response}' if not test_passed else '')
