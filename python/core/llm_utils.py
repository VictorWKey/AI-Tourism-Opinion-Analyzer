...
import time
import logging
from typing import TypeVar, Optional, Callable, Any, Type
...
def extraer_json_de_respuesta(texto: str) -> Optional[str]:
    """
...
def parsear_json_seguro(texto: str) -> Optional[dict]:
    """
...
def parsear_pydantic_seguro(
    texto: str, 
    modelo: Type[T],
    valores_default: Optional[dict] = None
) -> Optional[T]:
    """
            ...
            for intento in range(config.max_retries + 1):
                try:
                    ...
                    if intento < config.max_retries:
                        ...
                        logger.error(
                            f"Todos los reintentos agotados ({config.max_retries + 1} intentos). "
                            f"Último error: {e}"
                        )
            
            raise LLMRetryExhaustedError(
                f"Operación falló después de {config.max_retries + 1} intentos. "
...
def invocar_llm_con_retry(
    ...
    input_data: dict,
    max_retries: int = 3,
    modelo_pydantic: Optional[Type[T]] = None,
    valores_default: Optional[dict] = None
) -> Any:
    """
    ...
    raise LLMRetryExhaustedError(
        f"Operación LLM falló después de {max_retries + 1} intentos. "
        f"Último error: {ultimo_error}"
    )
...
def invocar_llm_con_fallback_manual(
    ...
    if valores_default:
        try:
            ...
            raise LLMRetryExhaustedError(
                f"No se pudo parsear respuesta ni usar valores default: {e}"
            )
