...
from .i18n import get_category_labels, get_sentiment_labels, get_translator
...
class GeneradorTemporal:
    ...
    def _generar_evolucion_sentimientos(self):
        ...
        for sentimiento in evol.columns:
            ax.plot(
                range(len(evol)),
                evol[sentimiento],
                label=sent_labels.get(sentimiento, sentimiento),
                color=COLORES_SENTIMIENTO.get(sentimiento, '#666'),
                marker='o',
                linewidth=2,
            )
    ...
    def _generar_tendencia_calificacion(self):
        ...
        mensual = (
            df_fechas.groupby('Mes')
            .agg(promedio=('Calificacion', 'mean'), conteo=('Calificacion', 'count'))
            .reset_index()
        )
        ...
        ax.axhline(
            ...
            alpha=0.5,
            linewidth=1.5,
            label=t('promedio_general', value=f'{promedio_global:.2f}'),
        )
