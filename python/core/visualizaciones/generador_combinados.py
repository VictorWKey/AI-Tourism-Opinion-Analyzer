...
class GeneradorCombinados:
    ...
    def _generar_sentimiento_subjetividad_categoria(self):
        ...
        # Heatmap
        sns.heatmap(
            contingencia, annot=True, fmt='d', cmap='YlGnBu', ax=ax, cbar_kws={'label': t('cantidad')}, linewidths=0.5
        )
    ...
    def _generar_volumen_vs_sentimiento(self):
        ...
        pct_positivo = [
            stats[c]['positivo'] / stats[c]['total'] * 100 if stats[c]['total'] > 0 else 0 for c in categorias
        ]
        pct_negativo = [
            stats[c]['negativo'] / stats[c]['total'] * 100 if stats[c]['total'] > 0 else 0 for c in categorias
        ]
        ...
        ax.scatter(
            volumenes,
            pct_positivo,
            s=[v * 3 for v in volumenes],
            c=colores,
            alpha=0.7,
            edgecolors=COLORES['borde_separador'],
            linewidth=1,
        )
        ...
        for i, cat in enumerate(categorias):
            ...
            ax.annotate(
                cat_display,
                (volumenes[i], pct_positivo[i]),
                textcoords='offset points',
                xytext=(5, 5),
                fontsize=FONT_SIZES['texto_pequeno'],
            )
    ...
    def _generar_correlacion_calificacion_sentimiento(self):
        ...
        parts = ax1.violinplot(
            [
                df_valid[df_valid['Sentimiento'] == s]['Calificacion'].values
                for s in orden_sent
                if s in df_valid['Sentimiento'].values
            ],
            positions=range(len(orden_sent)),
            showmeans=True,
            showmedians=True,
        )
        ...
        sns.heatmap(
            contingencia, annot=True, fmt='d', cmap='Blues', ax=ax2, cbar_kws={'label': t('cantidad')}, linewidths=0.5
        )
